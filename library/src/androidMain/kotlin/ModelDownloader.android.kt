    ...
    return withContext(Dispatchers.IO) {
        try {
            for (task in tasks) {
                ...
                val zipFilePath = File(File(applicationContext.filesDir, "models"), task.filename)
                zipFilePath.parentFile?.mkdirs()
                
                Log.d("CactusDownloader", "Downloading ZIP file from ${task.url}")
                val urlConnection = URL(task.url).openConnection() as HttpURLConnection
                urlConnection.connect()

                if (urlConnection.responseCode != HttpURLConnection.HTTP_OK) {
                    throw IOException("Failed to download ZIP file: ${urlConnection.responseCode} from ${task.url}")
                }
